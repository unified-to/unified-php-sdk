<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace Unified\Unified_to\Models\Shared;


class AdsAd
{
    /**
     *
     * @var ?string $adCopy
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('ad_copy')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $adCopy = null;

    /**
     *
     * @var ?AdType $adType
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('ad_type')]
    #[\Speakeasy\Serializer\Annotation\Type('\Unified\Unified_to\Models\Shared\AdType|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?AdType $adType = null;

    /**
     *
     * @var ?string $campaignId
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('campaign_id')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $campaignId = null;

    /**
     *
     * @var ?\DateTime $createdAt
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('created_at')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?\DateTime $createdAt = null;

    /**
     *
     * @var ?string $creativeAssetUrl
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('creative_asset_url')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $creativeAssetUrl = null;

    /**
     * $creativeIds
     *
     * @var ?array<string> $creativeIds
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('creative_ids')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string>|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?array $creativeIds = null;

    /**
     *
     * @var ?string $cta
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('cta')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $cta = null;

    /**
     *
     * @var ?string $description
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('description')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $description = null;

    /**
     *
     * @var ?string $finalUrl
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('final_url')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $finalUrl = null;

    /**
     *
     * @var ?string $groupId
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('group_id')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $groupId = null;

    /**
     *
     * @var ?string $headline
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('headline')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $headline = null;

    /**
     *
     * @var ?string $id
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('id')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $id = null;

    /**
     *
     * @var ?bool $isActive
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('is_active')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?bool $isActive = null;

    /**
     *
     * @var ?string $itemId
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('item_id')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $itemId = null;

    /**
     *
     * @var ?string $name
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('name')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $name = null;

    /**
     *
     * @var ?string $organizationId
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('organization_id')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $organizationId = null;

    /**
     * $raw
     *
     * @var ?array<string, mixed> $raw
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('raw')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string, mixed>|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?array $raw = null;

    /**
     *
     * @var ?AdsAdStatus $status
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('status')]
    #[\Speakeasy\Serializer\Annotation\Type('\Unified\Unified_to\Models\Shared\AdsAdStatus|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?AdsAdStatus $status = null;

    /**
     *
     * @var ?PropertyAdsAdTargeting $targeting
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('targeting')]
    #[\Speakeasy\Serializer\Annotation\Type('\Unified\Unified_to\Models\Shared\PropertyAdsAdTargeting|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?PropertyAdsAdTargeting $targeting = null;

    /**
     *
     * @var ?\DateTime $updatedAt
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('updated_at')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?\DateTime $updatedAt = null;

    /**
     * @param  ?string  $adCopy
     * @param  ?AdType  $adType
     * @param  ?string  $campaignId
     * @param  ?\DateTime  $createdAt
     * @param  ?string  $creativeAssetUrl
     * @param  ?array<string>  $creativeIds
     * @param  ?string  $cta
     * @param  ?string  $description
     * @param  ?string  $finalUrl
     * @param  ?string  $groupId
     * @param  ?string  $headline
     * @param  ?string  $id
     * @param  ?bool  $isActive
     * @param  ?string  $itemId
     * @param  ?string  $name
     * @param  ?string  $organizationId
     * @param  ?array<string, mixed>  $raw
     * @param  ?AdsAdStatus  $status
     * @param  ?PropertyAdsAdTargeting  $targeting
     * @param  ?\DateTime  $updatedAt
     * @phpstan-pure
     */
    public function __construct(?string $adCopy = null, ?AdType $adType = null, ?string $campaignId = null, ?\DateTime $createdAt = null, ?string $creativeAssetUrl = null, ?array $creativeIds = null, ?string $cta = null, ?string $description = null, ?string $finalUrl = null, ?string $groupId = null, ?string $headline = null, ?string $id = null, ?bool $isActive = null, ?string $itemId = null, ?string $name = null, ?string $organizationId = null, ?array $raw = null, ?AdsAdStatus $status = null, ?PropertyAdsAdTargeting $targeting = null, ?\DateTime $updatedAt = null)
    {
        $this->adCopy = $adCopy;
        $this->adType = $adType;
        $this->campaignId = $campaignId;
        $this->createdAt = $createdAt;
        $this->creativeAssetUrl = $creativeAssetUrl;
        $this->creativeIds = $creativeIds;
        $this->cta = $cta;
        $this->description = $description;
        $this->finalUrl = $finalUrl;
        $this->groupId = $groupId;
        $this->headline = $headline;
        $this->id = $id;
        $this->isActive = $isActive;
        $this->itemId = $itemId;
        $this->name = $name;
        $this->organizationId = $organizationId;
        $this->raw = $raw;
        $this->status = $status;
        $this->targeting = $targeting;
        $this->updatedAt = $updatedAt;
    }
}